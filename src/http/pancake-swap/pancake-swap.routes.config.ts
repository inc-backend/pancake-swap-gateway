...
import { JSBI, Pair, Token, TokenAmount, Trade } from '@ac32/spookyswap-sdk';
...
import {
  ...
  SPOOKY_MAINNET_CONFIGS,
  SPOOKY_TESTNET_CONFIGS,
} from './constants';
...
export class PancakeSwapRoutesConfig extends CommonRoutesConfig {
  async getBestRateFromPancake(params: {
    ...
    if (chainId == SPOOKY_MAINNET_CONFIGS.chainID) {
      bscConfigs = FTM_MAINNET_CONFIGS;
      pancakeConfigs = SPOOKY_MAINNET_CONFIGS;
    }

    if (chainId == SPOOKY_TESTNET_CONFIGS.chainID) {
      bscConfigs = FTM_TESTNET_CONFIGS;
      pancakeConfigs = SPOOKY_TESTNET_CONFIGS;
    }
